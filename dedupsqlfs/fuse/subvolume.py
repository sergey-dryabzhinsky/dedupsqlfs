...
class Subvolume(object):
    ...
    def create(self, name):
        ...
        tableTree = self.getTable('tree_%s' % subvolItem["id"])
        tableInode = self.getTable('inode_%s' % subvolItem["id"])
    ...
    def remove(self, name):
        ...
        try:
            self.getTable('tree_%s' % subvolItem["id"]).drop()
            self.getTable('inode_%s' % subvolItem["id"]).drop()
            self.getTable('inode_hash_block_%s' % subvolItem["id"]).drop()
            self.getTable('inode_option_%s' % subvolItem["id"]).drop()
            self.getTable('link_%s' % subvolItem["id"]).drop()
            self.getTable('xattr_%s' % subvolItem["id"]).drop()
    ...
    def get_usage(self, name, hashTypes=False):
        ...
        tableIndex = self.getTable('inode_hash_block_%s' % subvolItem["id"])
        tableTree = self.getTable('tree_%s' % subvolItem["id"])
        tableInode = self.getTable('inode_%s' % subvolItem["id"])
    ...
    def get_root_diff(self, name):
        ...
        tableRootIndex = self.getTable('inode_hash_block_%s' % subvolRootItem["id"])
        tableIndex = self.getTable('inode_hash_block_%s' % subvolItem["id"])
    ...
    def decompress_all_non_root_tables(self):
        ...
        for subvol_id in tableSubvol.get_ids('created_at'):
            ...
            for tn in tableNames:
                ...
                """
                table = self.getTable(tn + '_%s' % subvol["id"])
