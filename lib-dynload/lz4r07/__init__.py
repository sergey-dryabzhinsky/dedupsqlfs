...
build_dir = os.path.abspath( os.path.join(currentdir, "lib-dynload", "lz4r07", "build") )
...
for d in dirs:
    if d.find("-%s.%s" % (p1, p2)) != -1 and d.find("lib.") != -1:
        ...
        import imp
        fp, pathname, description = imp.find_module("_lz4r07")
        module = imp.load_module("_lz4r07", fp, pathname, description)
