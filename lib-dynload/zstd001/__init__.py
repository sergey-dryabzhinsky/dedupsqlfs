...
build_dir = os.path.abspath( os.path.join(currentdir, "lib-dynload", "zstd001", "build") )
...
for d in dirs:
    if d.find("-%s.%s" % (p1, p2)) != -1 and d.find("lib.") != -1:
        ...
        import imp
        fp, pathname, description = imp.find_module("_zstd001")
        module = imp.load_module("_zstd001", fp, pathname, description)
